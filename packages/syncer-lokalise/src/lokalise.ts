...
function addToArraySet<T>(array: T[], value: T): T[] {
  const set = new Set(array)
  set.add(value)
  return [...set]
}

function removeFromArraySet<T>(array: T[], value: T): T[] {
  const set = new Set(array)
  set.delete(value)
  return [...set]
}

function isPureKey(keyName: string, prefixes: string[]): boolean {
          ...
          if (tr.translation) {
            if (key.is_plural) {
              try {
                ...
                log.warn('updateTransEntries', `cannot parse translation object: ${tr.translation}`, err)
              }
            } else {
